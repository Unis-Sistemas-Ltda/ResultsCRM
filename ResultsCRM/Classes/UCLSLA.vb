Public Class UCLSLA
    ...
    Public Sub FillDropDown(ByRef DDL As DropDownList, ByVal AdicionarRegistroEmBranco As Boolean, ByVal DescricaoRegistroEmBranco As String)
        ...
        strSql += "   from sla "
        strSql += "  order by prazo_encerramento desc, descricao asc "
        dt = objAcessoDados.BuscarDados(strSql)
    ...
    Public Sub FillDropDown(ByRef DDL As DropDownList, ByVal AdicionarRegistroEmBranco As Boolean, ByVal DescricaoRegistroEmBranco As String, ByVal pCodCliente As String, ByVal pCodPais As String, ByVal pCodEstado As String, ByVal pCodCidade As String, ByVal pCodSLAAtual As String)
        Dim strSql As String = ""
        Dim dt As DataTable

        strSql += " call sp_sla_disponivel(" + pCodCliente + ", " + IIf(String.IsNullOrWhiteSpace(pCodPais), "0", pCodPais) + ", " + IIf(String.IsNullOrWhiteSpace(pCodEstado), "0", pCodEstado) + ", " + IIf(String.IsNullOrWhiteSpace(pCodCidade), "0", pCodCidade) + ", " + IIf(String.IsNullOrWhiteSpace(pCodSLAAtual), "0", pCodSLAAtual) + ")"
        dt = objAcessoDados.BuscarDados(strSql)
    ...
    Public Function GetQtdChamadosData(ByVal pCodCliente As String, ByVal pCodSLA As String, ByVal pData As Date) As Long
        Try
            If String.IsNullOrEmpty(pCodSLA) Then Return 0
            If String.IsNullOrEmpty(pCodCliente) Then Return 0
            Dim ret As Long = 0
            Dim strSql = " select count(1) qtd from chamado where cod_emitente = " + pCodCliente + " and date(data_chamado) = '" + pData.ToString("yyyy-MM-dd") + "' and cod_sla = " + pCodSLA
            Dim dt As DataTable = objAcessoDados.BuscarDados(strSql)
            If dt.Rows.Count > 0 Then
