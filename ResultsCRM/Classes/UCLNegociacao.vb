Public Class UCLNegociacao
    ...
    Public Sub EnviaNegociacaoPorEmail(ByVal GlClienteUnis As String, ByVal GlSMTP As String, ByVal GlPortaEnvio As String, ByVal GlSSL As String, ByVal CaminhoImagem As String)
        ...
        Try
            If String.IsNullOrEmpty(CaminhoImagem) Then
                If GlClienteUnis = "407" Then
                    CaminhoImagem = "http://187.121.81.208:50080/Imagens/logo_cliente.jpg"
                Else
                    CaminhoImagem = "http://localhost/" + System.Configuration.ConfigurationManager.AppSettings("caminho") + "/Imagens/logo_proposta.jpg"
                End If
            End If

            'strURL = "http://localhost:40721" + System.Configuration.ConfigurationManager.AppSettings("caminho") + "/Forms/WFImpressaoPropostaPDF.aspx?eid=" + Me.Empresa + "&sid=" + Me.Estabelecimento + "&nid=" + Me.CodNegociacao + "&ucid=" + GlClienteUnis + "&li=" + CaminhoImagem
            strURL = "http://localhost/" + System.Configuration.ConfigurationManager.AppSettings("caminho") + "/Forms/WFImpressaoPropostaPDF.aspx?eid=" + Me.Empresa + "&sid=" + Session("SEstabelecimentoNegociacao") + "&nid=" + Me.CodNegociacao + "&ucid=" + GlClienteUnis + "&li=" + CaminhoImagem
            textoEmail = GetHtmlFromAspx(strURL)

            Dim pos As Integer = textoEmail.IndexOf("<table")
            If pos >= 0 Then
                textoEmail = textoEmail.Substring(pos)
            End If
            pos = textoEmail.LastIndexOf("</table>")
            If pos >= 0 Then
                textoEmail = textoEmail.Substring(0, pos + 8)
            End If

            textoEmail = textoEmail.Replace(Chr(10), "")
            textoEmail = textoEmail.Replace(Chr(13), "")

            'Obtém email do destinatário
            If String.IsNullOrEmpty(Me.CodContato) OrElse Me.CodContato = "0" Then
                Throw New Exception("Contato não informado na negociação.")
                Return
            End If

            objContato.CodEmitente = Me.CodCliente
            objContato.Codigo = Me.CodContato
            objContato.Buscar()

            If String.IsNullOrEmpty(objContato.Email) Then
                Throw New Exception("E-mail não informado no cadastro do contato vinculado à negociação.")
                Return
            End If

            Para = objContato.Email.Trim.Replace(";", ",").Replace(" ", "")

            If Not String.IsNullOrWhiteSpace(Me.CodGestorConta) Then
                objGestor.Codigo = Me.CodGestorConta
                objGestor.Buscar()
                If Not String.IsNullOrWhiteSpace(objGestor.Email) Then
                    Para += ", " + objGestor.Email
                End If
            End If

            If objParametrosEmail.Buscar(Me.Empresa) Then
                anexo = objParametrosEmail.GetConteudo("anexo_negociacao")
            Else
                anexo = ""
            End If

            If Not String.IsNullOrEmpty(anexo) AndAlso Not String.IsNullOrEmpty(Me.AnexoProposta) Then
                anexo += ","
            End If

            If Not String.IsNullOrEmpty(Me.AnexoProposta) Then
                anexo += Me.AnexoProposta
            End If

            Dim anexos As String()
            If anexo.Contains(",") Then
                anexos = anexo.Split(",")
            ElseIf anexo.Length > 0 Then
                anexos = {anexo}
            Else
                anexos = {}
            End If
            ...
            Dim objParametrosManutencao = New UCLParametrosEmail
            objParametrosManutencao.Buscar(Session("GlEmpresa"))
            For i As Integer = 0 To anexos.Length - 1
                Dim anx As String = anexos(i)
                If Not anx.Contains(":") Then
                    anx = objParametrosManutencao.GetConteudo("diretorio_anexo_proposta") + IIf(objParametrosManutencao.GetConteudo("diretorio_anexo_proposta").ToString().EndsWith("\"), "", "\") + anx
                End If
                anexo += anx + IIf(i < anexos.Length - 1, ", ", "")
            Next

            Smtp = GlSMTP

            If String.IsNullOrEmpty(Smtp) Then
                Throw New Exception("Servidor SMTP não configurado nos parâmetros de e-mail.")
                Return
            End If

            If String.IsNullOrEmpty(Me.CodChamado) Then
                'Obtém dados do remetente para o envio do email
                objAgenteVendas.Codigo = Me.CodAgenteVenda
                objAgenteVendas.Buscar()

                De = objAgenteVendas.Email
                SenhaEmail = objAgenteVendas.Senha

                If String.IsNullOrEmpty(De) OrElse String.IsNullOrEmpty(SenhaEmail) Then
                    Throw New Exception("Endereço de e-mail/senha não informado no cadastro do agente de vendas vinculado à negociação.")
                    Return
                End If

                If Not De.IsValidEmail Then
                    Throw New Exception("Endereço de e-mail informado no cadastro do agente de vendas vinculado à negociação não é válido.")
                    Return
                End If

                objEmail.envia_email(Empresa, Smtp, De, Para, "Proposta Comercial", textoEmail, SenhaEmail, GlPortaEnvio, GlSSL, anexo)
            Else
                Dim objAnalista As New UCLAnalista
                objAnalista.Codigo = Me.CodAgenteVenda
                objAnalista.Buscar()

                De = objAnalista.Email
                SenhaEmail = objAnalista.SenhaEmail

                If String.IsNullOrEmpty(De) OrElse String.IsNullOrEmpty(SenhaEmail) Then
                    Throw New Exception("Endereço de e-mail/senha não informado no cadastro do analista do chamado vinculado à negociação.")
                    Return
                End If

                If Not De.IsValidEmail Then
                    Throw New Exception("Endereço de e-mail informado no cadastro do analista do chamado vinculado à negociação não é válido.")
                    Return
                End If
                ...
                objEmail2.SetConteudo("empresa", Me.Empresa)
                objEmail2.SetConteudo("seq_email", objEmail2.GetProximoCodigo(Me.Empresa))
                objEmail2.SetConteudo("remetente_nome", objAnalista.Remetente)
                objEmail2.SetConteudo("remetente_email", De)
                objEmail2.SetConteudo("destinatario_email", Para)
                objEmail2.SetConteudo("assunto", "Proposta Comercial")
                objEmail2.SetConteudo("corpo", textoEmail)
                objEmail2.SetConteudo("anexos", anexo)
                objEmail2.SetConteudo("prioridade", "1")
                objEmail2.SetConteudo("tipo_email", "1")
                objEmail2.SetConteudo("cod_usuario", Me.CodAgenteVenda)
                objEmail2.SetConteudo("situacao", "1")
                objEmail2.SetConteudoData("data_inclusao", Now())
                objEmail2.SetConteudoData("data_alteracao", Now())
                objEmail2.SetConteudo("descricao_erro", "")
                objEmail2.SetConteudo("data_mensagem", Now())
                objEmail2.SetConteudo("situacao_leitura", "0")
                objEmail2.SetConteudo("conta_principal", "N")
                objEmail2.Incluir()

                For Each arquivo As String In anexos
                    Dim dirProp As String = objParametrosEmail.GetConteudo("diretorio_anexo_proposta")
                    Dim dirEnv As String = objParametrosEmail.GetConteudo("diretorio_anexo_email_enviado")
                    If Not dirProp.EndsWith("\") Then
                        dirProp += "\"
                    End If
                    If Not dirEnv.EndsWith("\") Then
                        dirEnv += "\"
                    End If
                    If IO.File.Exists(dirProp + arquivo) Then
                        If Not IO.File.Exists(dirEnv + arquivo) Then
                            IO.File.Copy(dirProp + arquivo, dirEnv + arquivo)
                        End If
                    End If
                Next
            End If
    ...
    Public Sub EnviaEmailAlteracaoStatus()
        Try
            ...
            If String.IsNullOrEmpty(Me.CodAgenteVenda) Then
                Return
            End If

            If String.IsNullOrEmpty(Me.CodEtapaNegociacao) Then
                Return
            End If
            ...
            objEmitente.CodEmitente = Me.CodCliente
            If objEmitente.Buscar().Rows.Count = 0 Then
                Return
            End If

            objEtapaNegociacao.Codigo = Me.CodEtapaNegociacao
            objEtapaNegociacao.Empresa = Me.Empresa
            If Not objEtapaNegociacao.Buscar() Then
                Return
            End If

            objEmail.Empresa = Me.Empresa
            objEmail.Estabelecimento = Me.Estabelecimento
            objEmail.CodNegociacaoCliente = Me.CodNegociacao
            objEmail.Seq = objEmail.GetProximoCodigo()
            objEmail.Assunto = "Negociação Nº" + Me.CodNegociacao + " - Funil alterado "
            ...
            objEmail.Mensagem = "Prezado(a)<br><br>Informamos que a negociação nº " + Me.CodNegociacao + " do cliente " + objEmitente.Nome + "(" + objEmitente.CodEmitente + ") teve seu funil de vendas alterado para " + Chr(34) + objEtapaNegociacao.Descricao + Chr(34) + ".<br><br>Att,<br>" + objAgenteVendas.Nome
    ...
    Public Function TemItens() As Boolean
        Try
            Dim StrSql As String = "select count(1) qtd from negociacao_cliente_item where empresa = " + Empresa + " and estabelecimento = " + Estabelecimento + " and cod_negociacao_cliente = " + CodNegociacao
            Dim dt As DataTable = objAcessoDados.BuscarDados(StrSql)
            If dt.Rows.Count = 0 Then
                Return False
            Else
