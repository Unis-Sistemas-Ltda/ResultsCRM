Public Class UCLAdesaoTEF
    ...
    Public Function GetLojas(ByVal pCodAdesao As String) As List(Of StLoja)
        Try
            Dim StrSql As String = "select cod_emitente, (select first cnpj from adesao_tef_loja t where t.empresa = l.empresa and t.cod_emitente = l.cod_emitente) cnpj from adesao_tef_loja l where cod_emitente = 3616 and cod_adesao = " + pCodAdesao + " group by cod_emitente"
            Dim dt As DataTable = ObjAcessoDados.BuscarDados(StrSql)
            Dim loja As StLoja
    ...
    Public Function GetLojasPendentes(ByVal pCodAdesao As String) As List(Of StLoja)
        Try
            Dim StrSql As String = "select cod_emitente, (select first cnpj from adesao_tef_loja t where t.empresa = l.empresa and t.cod_emitente = l.cod_emitente) cnpj from adesao_tef_loja l where cod_negociacao_cliente is null and cod_adesao = " + pCodAdesao + " group by cod_emitente, empresa "
            Dim dt As DataTable = ObjAcessoDados.BuscarDados(StrSql)
            Dim loja As StLoja
            Dim ret As New List(Of StLoja)
