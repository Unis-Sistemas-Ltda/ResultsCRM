Public Class UCLSolicitacaoDesenvolvimento
    ...
    Public Property CodDesenvolvedor As String
    ...
    Public Function Buscar() As Boolean
        ...
        If dt.Rows.Count > 0 Then
            ...
            If String.IsNullOrWhiteSpace(Status) Then
                Status = "0"
            End If
            CodDesenvolvedor = dt.Rows.Item(0).Item("cod_desenvolvedor").ToString
            If String.IsNullOrWhiteSpace(CodDesenvolvedor) Then
                CodDesenvolvedor = "0"
            End If
    ...
    Public Sub Incluir()
        ...
        Try
            ...
            If String.IsNullOrEmpty(CodDesenvolvedor) OrElse CodDesenvolvedor = "0" Then
                CodDesenvolvedor = "null"
            End If

            strSql += " insert into solicitacao_desenvolvimento( empresa, cod_solicitacao, data_solicitacao, status, origem, cod_analista, cod_desenvolvedor, cod_emitente, cod_sistema, descricao, regra_negocio, urgencia, prazo_obrigatorio, data_obrigatoria, email_cliente_aprovar, assunto, versao," + Chr(34) + "release" + Chr(34) + ", previsao_entrega, data_proxima_visita, tipo)"
            strSql += " values (" + Empresa + ", " + CodSolicitacao + ", " + CDate(DataSolicitacao).ToString("yyyyMMdd") + ", " + Status + ", " + Origem + ", " + CodAnalista + ", " + CodDesenvolvedor + ", " + CodEmitente + ", " + CodSistema + ", '" + Descricao + "', '" + RegraNegocio + "', " + Urgencia + ",'" + PrazoObrigatorio + "', "
            ...
            If String.IsNullOrEmpty(Versao) Then
                strSql += "null, "
            Else
                strSql += Versao + ", "
            End If
    ...
    Public Sub Alterar()
        ...
        Try
            ...
            strSql += "        cod_desenvolvedor = " + CodDesenvolvedor + ", "
            strSql += "        cod_emitente      = " + CodEmitente + ", "
            strSql += "        cod_sistema       = " + CodSistema + ", "
            strSql += "        assunto           = '" + Assunto + "', "
            strSql += "        status            = " + Status + ", "
            strSql += "        tipo              = " + Tipo + ", "
            strSql += "        versao            = " + IIf(String.IsNullOrEmpty(Versao), "null", Versao) + ", "
            strSql += Chr(34) + "release" + Chr(34) + " = " + IIf(String.IsNullOrEmpty(Release), "null", Release) + ", "
