...
#Region "PublicProperties"
    ...
    Private _ValorCusto As Double
    Private _Margem As Double
    Private _Recurso As Double

    Public Property Recurso As Double
        Get
            Return _Recurso
        End Get
        Set(value As Double)
    ...
    Public Property ValorCusto As Double
        Get
            Return _ValorCusto
        End Get
        Set(value As Double)
...
#Region "Operations"
    Public Function CalculaTotais() As String
        ...
        Dim ld_aliquota_substituicao, ld_icms_auxiliar, ld_ipi_unitario, ld_icms_unitario As Double?
        ...
        If ll_cod_tabela Is Nothing Then
            ...
            strSql += " where empresa = " + Empresa.ToString
            strSql += "   and estabelecimento = " + Estabelecimento.ToString
            dt = objAcessoDados.BuscarDados(strSql)
            If dt.Rows.Count > 0 Then
                ll_cod_tabela = dt.Rows.Item(0).Item("cod_tabela").ToString
            Else
                Return "Problemas na pesquisa de ICMS. Não foi possível localizar tabela relacionada ao estabelecimento."
            End If
        ...
        'Busca a Margem
        '------------
        strSql = " select IsNull(margem_contribuicao, 0) margem"
        strSql += "  from item_estabelecimento"
        strSql += " where empresa         = " + Empresa.ToString
        strSql += "   and estabelecimento = " + Estabelecimento.ToString
        ...
        dt = objAcessoDados.BuscarDados(strSql)
        If dt.Rows.Count > 0 Then
            Margem = dt.Rows.Item(0).Item("margem")
        Else
            Margem = 0
        End If
        ...
        '------------
        'Busca o custo
        strSql = " select isnull(f_custo_item (" + Empresa.ToString + ", " + Estabelecimento.ToString + ", '" + CodItem + "'),0) valor_custo"
        strSql += "  from dummy;"
        dt = objAcessoDados.BuscarDados(strSql)
        If dt.Rows.Count > 0 Then
            ValorCusto = dt.Rows.Item(0).Item("valor_custo")
        Else
            ValorCusto = 0
        End If

        If ld_valor_ipi IsNot Nothing AndAlso ld_valor_ipi <> 0 Then
            ld_ipi_unitario = ld_valor_ipi / ld_qtd_pedida
        Else
            ld_ipi_unitario = 0
        End If

        If ld_valor_icms IsNot Nothing AndAlso ld_valor_icms <> 0 Then
            ld_icms_unitario = ld_valor_icms / ld_qtd_pedida
        Else
            ld_icms_unitario = 0
        End If

        Margem = (ValorCusto * Margem) / 100

        Recurso = ld_preco_unitario - (ValorCusto + ld_ipi_unitario + ld_icms_unitario + Margem)
