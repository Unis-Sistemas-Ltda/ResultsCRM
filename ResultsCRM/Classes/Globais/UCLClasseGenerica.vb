Public MustInherit Class UCLClasseGenerica
    ...
    Public ObjAcessoDados As New UCLAcessoDados(StrConexao)
    ...
    Public Overridable Function GetConteudoData(ByVal pNomeCampo As String) As DateTime
        Try
            Dim campo As UCLTabelaGenericaCampo = Campos.Find(Function(x) x.NomeCampo = pNomeCampo)
            If campo IsNot Nothing Then
                If campo.IsNull Then
    ...
    Public Overridable Sub SetConteudoData(ByVal pNomeCampo As String, ByVal pConteudo As DateTime)
        Try
            Dim campo As UCLTabelaGenericaCampo = Campos.Find(Function(x) x.NomeCampo = pNomeCampo)
            Dim campoPK As UCLTabelaGenericaCampo = CamposPK.Find(Function(x) x.NomeCampo = pNomeCampo)
            If campo IsNot Nothing Then
                campo.ConteudoData = pConteudo
                campo.IsNull = False
            Else
                Throw New Exception("Campo [" + pNomeCampo + "] não foi encontrado")
            End If
    ...
    Public Overridable Function Buscar()
        Try
