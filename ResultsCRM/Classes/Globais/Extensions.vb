Module Extensions
    ...
    <System.Runtime.CompilerServices.Extension()> Public Function RetiraAspas(ByVal s As String) As String
        Return TiraAspa(s)

        Return True
    End Function

    <System.Runtime.CompilerServices.Extension()> Public Function PoeAspasSeHouver(ByVal s As String) As String
        Return PoeAspa(s)
    ...
    <System.Runtime.CompilerServices.Extension()> Public Function MascaraCNPJ(ByRef s As String) As String
        If String.IsNullOrEmpty(s) OrElse s.Length <= 3 Then
            Return s
        End If
    ...
    <System.Runtime.CompilerServices.Extension()> Public Function MascaraCPF(ByRef s As String) As String
        If String.IsNullOrEmpty(s) OrElse s.Length <= 3 Then
            Return s
        Else
            If IsNumeric(s) AndAlso s > 0 Then
    ...
    <System.Runtime.CompilerServices.Extension()> Public Function IsValidCNPJ(ByVal strCNPJ As String) As Boolean
        ...
        Select Case strCNPJ
            Case "00000000000000", "11111111111111", "22222222222222", "33333333333333", "44444444444444", "55555555555555", "66666666666666", "77777777777777", "88888888888888", "99999999999999"
                Return False
        End Select
    ...
    <System.Runtime.CompilerServices.Extension()> Public Function IsValidCPF(ByVal s As String) As Boolean
        Dim valor As String = s.Replace(".", "").Replace("-", "")
    ...
    Private Function TiraAspa(ByVal s As String) As String
        s = s.Replace("'", "`")
        Return s
    End Function

    Private Function PoeAspa(ByVal s As String) As String
        s = s.Replace("`", "'")
        Return s
    End Function
    ...
    <System.Runtime.CompilerServices.Extension()> Public Function FormataDataBanco(ByVal strData As String) As String
        Dim strDataBanco As String = ""

        If IsDate(strData) Then
            strDataBanco = strData.Substring(6, 4) & "-" & strData.Substring(3, 2) & "-" & strData.Substring(0, 2)
        End If

        Return strDataBanco
    End Function

    <System.Runtime.CompilerServices.Extension()> Public Function EmptyStringIsNull(ByVal s As String) As String
        If s Is Nothing Then
            s = "nulL"
        ElseIf String.IsNullOrWhiteSpace(s) Then
            s = "null"
        End If
    ...
    <System.Runtime.CompilerServices.Extension()> Public Function AdicionarHorasUteis(ByVal d As DateTime, ByVal pQtdHoras As String, ByVal pEmpresa As String, ByVal pEstabelecimento As String) As DateTime
        Try
            Dim objAcessoDados As New UCLAcessoDados(StrConexao)
            Dim StrsSql As String = "select isnull(f_calcula_prazo(" + IIf(String.IsNullOrEmpty(pEmpresa), "null", pEmpresa) + ", " + IIf(String.IsNullOrEmpty(pEstabelecimento), "null", pEmpresa) + ", '" + d.ToString("yyyy-MM-dd HH:mm:ss") + "', 'h', " + IIf(String.IsNullOrEmpty(pQtdHoras), "null", pQtdHoras) + ", 'u'), '" + d.ToString("yyyy-MM-dd HH:mm:ss") + "') data_calculada from dummy"
            Dim dt As DataTable = objAcessoDados.BuscarDados(StrsSql)
            If dt.Rows.Count > 0 Then
                Return CType(dt.Rows.Item(0).Item("data_calculada"), DateTime)
            Else
                Return d
            End If
        Catch ex As Exception
