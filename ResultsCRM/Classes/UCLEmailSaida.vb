Public Class UCLEmailSaida
    ...
    Public Sub Incluir()
        Try
            ...
            strSql += "insert into email_saida(email, data_email, de, remetente, para, destinatario, assunto, mensagem, anexo, enviado, empresa, cod_chamado, enviar_automatico, enviado_automatico, data_envio_automatico, destinatario_cc, destinatario_cco) "
            strSql += " values(" + Email + ", "
    ...
    Public Sub EnviaEmailsPendentesDoAnalista(ByVal pEmpresa As String, ByVal pAnalistaChamado As String)
        Try
            Dim strSql As String = ""
            strSql += "  update email_saida e "
            strSql += "     set e.enviado = getdate(), "
            strSql += "         e.enviado_automatico = 'N' "
            strSql += "    from chamado c "
            strSql += "   where e.empresa         =  " + Empresa
            strSql += "     and c.cod_analista    =  " + pAnalistaChamado
            strSql += "     and e.enviado         is null "
            strSql += "     and c.empresa         = e.empresa "
            strSql += "     and c.cod_chamado     = e.cod_chamado"
            strSql += "     and enviar_automatico = 'S'"
            objAcessoDados.ExecutarSql(strSql)
        Catch ex As Exception
            Throw ex
