...
Partial Public Class WFRelResumoAcoesColaborador
    ...
    Protected Sub CarregaGrid()
        ...
        strSql += "select tt.nome_usuario usuario, count(tt.cod_acao) qtd_acao FROM ("
        strSql += "select s.nome_usuario, f.cod_acao , n.cod_negociacao_cliente, f.seq_follow_up, f.cod_usuario "
        ...
        End If
        strSql += " UNION ALL "
        strSql += "select distinct s.nome_usuario, f.cod_acao, f.cod_chamado, f.seq_follow_up, f.cod_usuario "
        ...
        strSql += " and f.data_follow_up between '" + dataIni + "' and '" + dataFim + "'"
        If Not String.IsNullOrEmpty(TxtEmitente.Text) Then
            strSql += " and isnull(e.nome,'Sem Negociacao') like '%" + TxtEmitente.Text + "%'"
        End If
        If ddlEtapa.SelectedValue <> 0 Then
            strSql += " and isnull(n.cod_etapa,0) = " + ddlEtapa.SelectedValue
        End If
        If ddlFunil.SelectedValue <> 0 Then
            strSql += " and isnull(n.cod_funil,0) = " + ddlFunil.SelectedValue
        End If
        If LbxUsuario.SelectedValue <> 0 Then
            strSql += " and f.cod_usuario in (" + usuarios + ")"
        End If
        If LbxCanalVenda.SelectedValue <> 0 Then
            strSql += " and isnull(n.cod_canal_venda,0) in (" + canais + ")"
        End If
        strSql += " ) as tt"
        strSql += " group by tt.nome_usuario"
        strSql += " order by tt.nome_usuario"
