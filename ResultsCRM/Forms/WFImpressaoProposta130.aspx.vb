Partial Public Class WFImpressaoProposta130
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        GridView1.DataBind()
        SqlDataSourceDespesas.Select(DataSourceSelectArguments.Empty)
    ...
    Private Sub CarregaFormulario()
        ...
        If dt.Rows.Count > 0 Then
            ...
            If Not String.IsNullOrEmpty(dt.Rows.Item(0).Item("detalhes_formulacao").ToString) Then
                LblTxtFormulacoes.Visible = True
                LblDetalhesFormulacoes.Text = dt.Rows.Item(0).Item("detalhes_formulacao").ToString
            Else
                LblTxtFormulacoes.Visible = False
                LblDetalhesFormulacoes.Visible = False
            End If
            If Not String.IsNullOrEmpty(dt.Rows.Item(0).Item("detalhes_embalagem").ToString) Then
                LblTxtEmbalagens.Visible = True
                LblDetalhesEmbalagem.Text = dt.Rows.Item(0).Item("detalhes_embalagem").ToString
            Else
                LblTxtEmbalagens.Visible = False
                LblDetalhesEmbalagem.Visible = False
            End If
            If Not String.IsNullOrEmpty(dt.Rows.Item(0).Item("detalhes_rotulos").ToString) Then
                LblTxtRotulos.Visible = True
                LblDetalhesRotulos.Text = dt.Rows.Item(0).Item("detalhes_rotulos").ToString
            Else
                LblTxtRotulos.Visible = False
                LblDetalhesRotulos.Visible = False
            End If
            If Not String.IsNullOrEmpty(dt.Rows.Item(0).Item("detalhes_prazo_entrega").ToString) Then
                LblTxtPrazoEntrega.Visible = True
                LblDetalhesPrazoEntrega.Text = dt.Rows.Item(0).Item("detalhes_prazo_entrega").ToString
            Else
                LblTxtPrazoEntrega.Visible = False
                LblDetalhesPrazoEntrega.Visible = False
            End If
            If Not String.IsNullOrEmpty(dt.Rows.Item(0).Item("detalhes_pagamento").ToString) Then
                LblTxtDetalhesPagamento.Visible = True
                LblDetalhesPagamento.Text = dt.Rows.Item(0).Item("detalhes_pagamento").ToString
            Else
                LblTxtDetalhesPagamento.Visible = False
                LblDetalhesPagamento.Visible = False
            End If
        ...
        '    If row.RowType = DataControlRowType.DataRow Then
        '        Dim codItem2 = CType(row.FindControl("LblCodDespesa"), Label).Text
        '        Dim valor2 = CDbl(CType(row.FindControl("LblValorDespesa"), Label).Text)
        '    End If
        'Next
