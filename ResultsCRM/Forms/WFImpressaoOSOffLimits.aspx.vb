Partial Public Class WFImpressaoOSOffLimits
    ...
    Private Sub GridView2_DataBound(sender As Object, e As System.EventArgs) Handles GridView2.DataBound
        Dim valorTotalOS As Double = 0
        Dim strValor As String
        For Each row As GridViewRow In CType(sender, GridView).Rows
            strValor = CType(row.FindControl("LblValorTotalItem"), Label).Text
            If Not String.IsNullOrEmpty(strValor) AndAlso IsNumeric(strValor) Then
                valorTotalOS += CDbl(strValor.Replace(".", "").Replace(",", "."))
            End If
        Next
        LblValorTotal.Text = valorTotalOS.ToString("F2")
    End Sub
